...
def percision_recall_curve(plot_name, include_files, visualise_scoring_approach):
    ...
    for filename in os.listdir("trec_eval_results/"):
        if include_files in filename:
            f = open("trec_eval_results/%s" % filename, "r")
            data = f.read().strip()
            f.close()
...
percision_recall_curve('Recall-Precision Curve for Different Analyzers', "BM25Similarity", True)
